...
export const useSSE = ({
  ...
  const connect = useCallback(() => {
    if (!enabledRef.current || eventSourceRef.current) {
      return;
    }
    ...
    const url = new URL('/api/v1/sse/events', getApiBasePath());
    if (filters) {
    ...
    try {
      ...
      eventSource.onerror = (event) => {
        ...
        setRetryCount(prev => {
          ...
          if (enabledRef.current && currentRetryCount < maxRetries) {
            ...
            if (currentRetryCount >= maxRetries) {
              setError(`Max retry attempts (${maxRetries}) exceeded`);
            }
      ...
      if (workflowId) {
        ...
        eventSource.addEventListener(jobEventType, (event) => {
          try {
            ...
            console.error(`❌ Failed to parse ${jobEventType} data:`, parseError);
          }
        });
        ...
        eventSource.addEventListener(workflowEventType, (event) => {
          try {
            const parsedData = JSON.parse(event.data) as DatabaseChangeData;
            console.log(`🔄 Workflow event for workflow ${workflowId}:`, parsedData);
            ...
            console.error(`❌ Failed to parse ${workflowEventType} data:`, parseError);
          }
        });
      }
  ...
  }, [filters, workflowId, enabled, reconnect]);
...
export const useLogSSE = (
  ...
  const connect = useCallback(() => {
    if (!enabledRef.current || eventSourceRef.current || !workflowId) {
      return;
    }
    ...
    const url = new URL(`/api/v1/logs/${workflowId}/sse`, getApiBasePath());
    ...
    try {
      ...
      eventSource.onopen = () => {
        ...
        setError(null);
        setRetryCount(0);
        console.log(`✅ Logs SSE connected successfully for workflow: ${workflowId}`);
      };
  ...
  }, [workflowId, maxRetries, reconnectInterval]);
