...
@pytest.mark.asyncio
async def test_create_and_list_catalogs(
    ...
    create_resp = await client.post(
        "/api/v1/catalog", json=create_req, headers=superuser_token_headers
    )
    ...
    update_resp = await client.patch(
        f"/api/v1/catalog/{slug}", json=update_req, headers=superuser_token_headers
    )
    ...
    write_resp = await client.put(
        f"/api/v1/catalog/{slug}/files/Snakefile",
        json=file_req,
    ...
    read_resp = await client.get(
        f"/api/v1/catalog/{slug}/files/Snakefile", headers=superuser_token_headers
    )
    ...
    dir_resp = await client.post(
        f"/api/v1/catalog/{slug}/dirs/test_dir", headers=superuser_token_headers
    )
    assert dir_resp.status_code == 200

    # 8. Delete directory
    del_dir_resp = await client.delete(
        f"/api/v1/catalog/{slug}/dirs/test_dir", headers=superuser_token_headers
    )
    ...
    # 9. Rename
    await client.put(
        f"/api/v1/catalog/{slug}/files/test.txt",
        json={"content": "hello"},
        headers=superuser_token_headers,
    )
    rename_resp = await client.post(
        f"/api/v1/catalog/{slug}/rename",
        json={"old_path": "test.txt", "new_path": "test2.txt"},
        headers=superuser_token_headers,
    )
    assert rename_resp.status_code == 200

    # 10. Delete file
    del_file_resp = await client.delete(
        f"/api/v1/catalog/{slug}/files/test2.txt", headers=superuser_token_headers
    )
