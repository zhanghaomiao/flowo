...
def test_prepare_url_inserts_token_correctly(git_service, monkeypatch):
    monkeypatch.setattr(
        "app.services.third_party.git.settings.CATALOG_GIT_TOKEN", "default_token"
    )

    # Custom token takes precedence
    assert (
        git_service._prepare_url("https://github.com/repo.git", "my_token")
        == "https://my_token@github.com/repo.git"
    )

    # Fallback to default token
    assert (
        git_service._prepare_url("https://github.com/repo.git")
        == "https://default_token@github.com/repo.git"
    )

    # Do not inject if already has auth
    assert (
        git_service._prepare_url("https://user:pass@github.com/repo.git", "token")
        == "https://user:pass@github.com/repo.git"
    )

    # Do not inject for ssh
    assert (
        git_service._prepare_url("git@github.com:repo.git", "token")
...
def test_test_connection_success(git_service):
    with patch("subprocess.run") as mock_run:
        ...
        success, msg = git_service.test_connection(
            "https://github.com/repo.git", "token"
        )
...
def test_init_repository_success(git_service, tmp_path, monkeypatch):
    monkeypatch.setattr(
        "app.services.third_party.git.settings.CATALOG_GIT_REMOTE",
        "https://github.com/repo.git",
    )
    monkeypatch.setattr(
        "app.services.third_party.git.settings.CATALOG_GIT_TOKEN", "token"
    )
    ...
    with patch.object(git_service, "_run_git") as mock_run_git:
        ...
        assert (
            mock_run_git.call_count >= 5
        )  # init, remote add, config email, config name, checkout
...
def test_push_to_monorepo(git_service, tmp_path):
    ...
    with (
        ...
        git_service.push_to_monorepo(
            tmp_path, "https://github.com", "token", "main", "my commit"
        )
...
def test_import_catalogs_from_own_monorepo(git_service, tmp_path, monkeypatch):
    ...
    with (
        patch.object(git_service, "init_repository"),
        patch.object(git_service, "_run_git") as mock_run_git,
        patch.object(
            git_service, "_scan_catalogs", return_value=["catalog1", "catalog2"]
        ) as mock_scan,
    ):
