...
class LogHandler(LogHandlerBase, PostgresqlLogHandler):
    ...
    def __post_init__(self) -> None:
        ...
        self.file_handler.setFormatter(DefaultFormatter(
            self.common_settings.quiet, self.common_settings.show_failed_logs, self.common_settings.printshellcmds))
        self.file_handler.addFilter(DefaultFilter(
            self.common_settings.quiet, self.common_settings.debug_dag, self.common_settings.dryrun))
        self.file_handler.setLevel(logging.DEBUG if self.common_settings.verbose else logging.INFO)
