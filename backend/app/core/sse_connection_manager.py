...
class SSEConnectionManager:
    ...
    async def connect_to_database(self):
        ...
        async with self._connection_lock:
            ...
            try:
                ...
                self.connection = await asyncpg.connect(settings.SQLALCHEMY_DATABASE_URI)
                
                await self.connection.add_listener('table_changes', self.handle_database_notification)
                await self.connection.add_listener('table_changes_workflows', self.handle_database_notification)
                await self.connection.add_listener('table_changes_jobs', self.handle_database_notification)
                
                logger.info("SSE: Connected to database and listening for notifications")
                
                if self._keep_alive_task:
                    self._keep_alive_task.cancel()
                self._keep_alive_task = asyncio.create_task(self.keep_connection_alive())
    ...
    def handle_database_notification(self, connection, pid, channel, payload):
        ...
        try:
            ...
            data['formatted_timestamp'] = datetime.fromtimestamp(data['timestamp']).isoformat()
    ...
    def broadcast(self, event: dict, workflow_id: Optional[str] = None):
        """
        ...
        logger.debug(f"SSE: Event broadcasted to {broadcast_type} clients: {broadcast_count}/{len(target_clients)} (filtered)")

    
    async def add_client(self, client_id: str, filters: Set[str], workflow_id: Optional[str] = None):
        ...
        logger.info(f"SSE: Added client {client_id} with filters: {filters}{workflow_info} (Total: {total_clients})")
    ...
    async def get_connection_stats(self):
        ...
        return {
            ...
            'database_connected': self.connection is not None and not self.connection.is_closed(),
            'client_filters': {client_id: list(filters) for client_id, filters in self.client_filters.items()},
            'workflow_subscriptions': workflow_stats,
            'keep_alive_task_running': self._keep_alive_task is not None and not self._keep_alive_task.done(),
